...
> **한계를 알고 쓰세요.** EC2/VPC 의 일부 생성 API 는 태그 조건을 받지 않아 `ec2:*` 를 열어야 합니다.
> 그래서 삭제·변경 계열에 Deny 를 걸어 막았는데, 이 Deny 는 **기존 프로젝트 리소스에 태그가 있을 때** 확실합니다.
> 기존 리소스가 태그 없이 방치돼 있다면, 그 VPC id 를 알려주시면 해당 ARN 을 콕 집어 Deny 에 넣겠습니다.
> 완전한 격리가 필요하면 AWS Organizations 로 **계정을 분리**하는 것이 유일한 확실한 답입니다.
