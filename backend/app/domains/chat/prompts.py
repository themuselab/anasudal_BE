...
# 후속 칩의 마지막 방어선. 평소에는 suggested_prompt 표(slot='followup')에서 꺼내 쓰고,
# 표가 비었거나 DB 가 흔들릴 때만 이 값이 나간다 — 칩이 없다고 대화가 막다른 길이 되면 안 된다.
# 칩을 누르면 그 문장이 그대로 질문으로 전송되므로 "말해볼게요" 같은 메타 문구는 쓸 수 없다.
STARTERS = ["아이가 말이 느린 것 같아요", "또래와 어울리는 걸 어려워해요"]
