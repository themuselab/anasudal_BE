...
# 답변 뒤에 붙이는 시작 질문. 칩을 누르면 그 문장이 그대로 질문으로 전송되므로
# "말해볼게요" 같은 메타 문구를 넣으면 안 된다 — 그 자체가 질문이 되어 또 헛돈다.
# 실제로 답이 나오는 문장이어야 상담으로 이어진다.
STARTERS = ["아이가 말이 느린 것 같아요", "또래와 어울리는 걸 어려워해요"]
STARTERS_ALT = ["아이가 말이 느린 것 같아요", "이름을 불러도 잘 쳐다보지 않아요"]
